...
def receive_url():
    try:
        ...
        for img_tag in img_tags:
            ...
            if(count<2):
                 predicted_product,percent = predict_product(img_url)
                 print("Hii..",percent)
        ...
        if(p[0]==p[1] and predicted_logo==p[1]):
            ...
            print("Threshold - ",str(percent))
            if(tval>=0):
                return jsonify({'status': 'success', 'message': f'Received Image URL: {img_url}', 'predicted_product': p[0].upper() , 'percent':str(percent)})
            else:
